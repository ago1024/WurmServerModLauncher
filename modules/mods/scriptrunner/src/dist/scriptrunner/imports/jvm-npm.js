...
/**
 * Changes by ago:
 * * uses ScriptRoot and ImportPaths from ScriptRunner
 * * Does not descend into parent dirs
 * * ./id resolves only local files
 * * id resolves only files from import paths
 */
...
(function () {
  ...
  var log = Logger.getLogger('scriptrunner.jvm-npm');
  ...
  function Require (id, parent) {
    ...
    if (!file) {
      if (typeof NativeRequire.require === 'function') {
        if (Require.debug) {
          log.info(['Cannot resolve', id, 'defaulting to native'].join(' '));
        }
        native_ = NativeRequire.require(id);
        if (native_) return native_;
      }
      log.severe('Cannot find module ' + id);
    ...
    try {
      ...
      if (ex instanceof java.lang.Exception) {
        ...
        log.info('Cannot load module ' + id + ' LOAD_ERROR');
        throw ex;
      }
    }
  }
  ...
  Require.root = ScriptRunner && ScriptRunner.ScriptRoot || System.getProperty('user.dir');
  Require.NODE_PATH = undefined;

  function findRoots (id, parent) {
    var pathParts = id.split(/[\/|\\,]+/g);
    if (pathParts.length > 0 && pathParts[0] === '.') {
      return [findRoot(parent)];
    } else {
      return [].concat(Require.paths());
    }
  }
  ...
  Require.paths = function () {
    var r = [];
    if (ScriptRunner && ScriptRunner.ImportPath) {
      r.push(ScriptRunner.ImportPath);
    }
    if (ScriptRunner && ScriptRunner.ImportPaths) {
      for (i in ScriptRunner.ImportPaths) {
        r.push(ScriptRunner.ImportPaths[i]);
      }
    }
